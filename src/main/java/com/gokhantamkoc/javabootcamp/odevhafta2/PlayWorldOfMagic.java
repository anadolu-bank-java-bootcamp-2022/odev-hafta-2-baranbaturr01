...
public class PlayWorldOfMagic implements CommandLineRunner {
    ...
    public static int resolveBattle(
        ...
        for (int i = 0; i < bossNames.length; i++) {
            float bossHP = bossHPs[i];
            float[] spellDamage = new float[magicianSpells.length];
            for (int j = 0; j < spellDamage.length; j++) {
                spellDamage[j] = spellDamageInfo[j];
            }
            int spellIndex = 0;
            while (bossHP > 0 && spellIndex < spellDamage.length) {
                float spellDamageAmount = spellDamage[spellIndex];
                bossHP -= spellDamageAmount;
                spellIndex++;
                spellsUsed++;
            }
        }
